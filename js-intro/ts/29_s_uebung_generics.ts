...
class Wrapper<O extends object> {
  #object: O;

  constructor(anObject: O) {
    this.#object = anObject;
  }

  getProperty<N extends keyof O>(name: N): O[N] {
    return this.#object[name];
  }

  addListener<N extends keyof O>(name: N, listenerCallback: (newValue: O[N]) => void) {
    // musst Du nicht implementieren :)
  }
}
